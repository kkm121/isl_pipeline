...
class ExperimentRegistry:
    ...
    def _get_commit_sha(self) -> str:
        try:
            res = subprocess.run(['git', 'rev-parse', 'HEAD'], cwd=self.project_root, capture_output=True, text=True, check=True)
    ...
    def record_metrics(self, experiment_id: str, metrics: Dict[str, float]):
        prov = self.get_experiment(experiment_id)
        prov.metrics.update(metrics)
        self._save_prov(prov)

    def record_diagnosis(self, experiment_id: str, diagnosis: Dict):
        prov = self.get_experiment(experiment_id)
        prov.diagnosis = diagnosis
        self._save_prov(prov)

    def complete_experiment(self, experiment_id: str, metrics: Dict, checkpoint_path: Optional[str] = None):
        prov = self.get_experiment(experiment_id)
    ...
    def fail_experiment(self, experiment_id: str, diagnosis: Dict):
        prov = self.get_experiment(experiment_id)
        prov.status = 'failed'
        prov.diagnosis = diagnosis
        self._save_prov(prov)
    ...
    def _save_prov(self, prov: ExperimentProvenance):
