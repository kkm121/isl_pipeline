...
def read_file(path: str) -> str:
    """Read file content (Read-only, allowed across all pipeline states)."""
...
def write_file(path: str, content: str) -> str:
    ...
    - Test files (tests/*) may be created during TEST_PLAN, IMPLEMENTATION, or RETRY.
    - Source/config files may only be modified during IMPLEMENTATION or RETRY.
    """
    safe_path = validate_path(path)
    relative = safe_path.relative_to(PROJECT_ROOT)

    is_test = bool(relative.parts and relative.parts[0] == "tests")
